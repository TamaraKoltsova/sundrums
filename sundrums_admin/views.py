...
def tipe_posts_entsiklopediia(request):
    ...
    Post_categoriess = Post_categories.objects.filter(is_active=True)
    #Post_categories_tipe = Post_categories.objects.get(id = post_id)
    Post_categories_tipe = Posts.objects.filter(categories_id = 7)
    Post_categories_name = Post_categories.objects.get(id = 7)
    reviewss = reviews.objects.filter(is_active=True)
    socbuttons = socbutton.objects.filter(is_active=True)
    tipe_kurss = tipe_kurs.objects.filter(is_active=True)
    return render(request, 'tipe_posts.html', locals())#
...
def perezapolnit(request):
    ...
    Posts_mass = Post_categories.objects.filter(is_active=True)
    #print(Posts_mass)
    for post in Posts_mass:
        Post_categories.objects.filter(id = str(post.id)).update(descrioptions_for_title = str(post.name), descrioptions_for_seo = str(post.name), descrioptions_for_descriptions = str(post.name) )
